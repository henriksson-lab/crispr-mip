...
def umicount_per_grna(tab):
    ...
    clustered_umis = clusterer(umicount_bytes, threshold=2)
...
def dedup_full_table(table_reads):
    ...
    deduped = todedup.apply(umicount_per_grna)
...
if True:
    ...
    for cursize in subsample_size_list:
        ...
        if cursize < table_reads.shape[0]:
            ...
            for subsamp_rng in range(0,num_subsamp):
                ...
                d.to_csv("/home/mahogny/mystore/dataset/crispr_padlock/subsamp/"+cursamp+
    ...
    if False:
        ...
        d = pd.DataFrame({"grna":[x[0] for x in list(counts.index)], "cnt":list(counts)})
        #d.to_csv("/corgi/otherdataset/crispr_padlock/newcount/padlock/"+cursamp+".nodedup.csv", index=False)    


        if table_reads.shape[0]>0:
            d = dedup_full_table(table_reads) 
            #d.to_csv("/corgi/otherdataset/crispr_padlock/newcount/padlock/"+cursamp+".dedup.csv", index=False)
