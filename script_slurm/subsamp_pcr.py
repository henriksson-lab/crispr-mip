...
grnalist = pd.read_csv("/cfs/klemming/home/j/johenr/mystore/crispr_padlock/lib/brunello.csv",sep="\t")
...
def readFastqPCR(seqR1, seqR2, seqR1b, seqR2b, set_grna):
    ...
    print("reading fastq")
    grna = readBufferToGrna(seqR1)
    print("done reading fastq")

    ### If resequenced, add additional reads
    if seqR1b!="":
         print("Adding resequencing data")
         grna = grna + readBufferToGrna(seqR1b) #note: there was a bug here; might it have affected?

    ### Store list of bad gRNAs for diagnostics
    bad_grna = [x for x in grna if x not in set_grna]
    table_reads = pd.DataFrame({"grna": bad_grna})
    table_reads.to_csv("/cfs/klemming/home/j/johenr/mystore/crispr_padlock/prob.csv", index=False)

    ### Only keep known grnas in table
...
if True:
    ...
    for cursize in subsample_size_list:
        ...
        if cursize < table_reads.shape[0]:
            ...
            #Set number of resamplings for this depth. only 1 needed if using all reads
            num_subsamp = 5
            if cursize==0:
                num_subsamp = 1
            ...
            for subsamp_rng in range(0,num_subsamp):
                ...
                d.to_csv("/cfs/klemming/home/j/johenr/mystore/crispr_padlock/subsamp/"+cursamp+"_pcr#"+str(cursize)+"#"+str(subsamp_rng), index=False)
